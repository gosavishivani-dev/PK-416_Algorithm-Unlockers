...
def predict_cropArrival(request):
    try:
        market='Bengaluru'
        commodity='Onion'
        data = pd.read_csv("Arrival_pred_Final.csv", index_col ="Market")
        s=data.loc[[market]]
        s.to_csv("Arrival1_23.csv", sep=',')
        data1 = pd.read_csv("Arrival1_23.csv", index_col ="Commodity")
        t=data1.loc[[commodity]]
        t.to_csv("Arrival_234.csv", sep=',')
        data2 = pd.read_csv("Arrival_234.csv")
        data2 = data2.drop(data2.columns.difference(["Unnamed: 0.1.1","Commodity","Market","Arrival"]), axis=1)
        #print(data2)
        data2.to_csv("final_arrival.csv",sep=',')
        pd.options.mode.chained_assignment = None
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        output =[]
        with open("final_arrival.csv",'r') as f:
            reader = csv.DictReader(f)
            for records in reader:
                output.append(records)
        with open("arrivalRecordsJsonModal.json",'w')as outfile:
            json.dump(output,outfile,sort_keys=True, indent=4)
        with open("arrivalRecordsJsonModal.json",'r')as infile:
